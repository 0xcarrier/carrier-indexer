...
import { BytesLike, arrayify, hexValue, hexZeroPad, zeroPad, keccak256 as ethersKeccak256 } from "ethers/lib/utils";
...
const TESTNET = {
  ...
  sui: {
    ...
    token_bridge: "0x6fb10cdb7aa299e9a4308752dadecb049ff55a892de92992a1edbd7912b3d6da",
    nft_bridge: undefined,
  },
  moonbeam: {
...
const DEVNET = {
  ...
  sui: {
    ...
    token_bridge: "0xa6a3da85bbe05da5bfd953708d56f1a3a023e7fb58e5a824a3d4de3791e8f690", // token_bridge module State object ID
    nft_bridge: undefined,
  },
  moonbeam: {
    core: undefined,
...
export const tryNativeToHexString = (address: string, chain: ChainId | ChainName): string => {
...
export function keccak256(data: BytesLike): Buffer {
  return Buffer.from(arrayify(ethersKeccak256(data)));
}
...
export function deriveAddress(seeds: (Buffer | Uint8Array)[], programId: PublicKeyInitData): PublicKey {
...
export const tryUint8ArrayToNative = (a: Uint8Array, chain: ChainId | ChainName): string => {
  const chainId = coalesceChainId(chain);
  if (isEVMChain(chainId)) {
...
export const tryHexToNativeString = (h: string, c: ChainId | ChainName): string =>
...
export function parseTokenTransferPayload(payload: Buffer): TokenTransfer {
  ...
  if (payloadType != TokenBridgePayload.Transfer && payloadType != TokenBridgePayload.TransferWithPayload) {
  ...
  const fee = payloadType == 1 ? BigInt(new BN(payload.subarray(101, 133)).toString()) : null;
