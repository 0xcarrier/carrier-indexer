...
function getResponse(request, json) {
  return new Promise(function (_resolve, _reject) {
    function resolve(resp) {
      console.timeEnd(`rpc time ${request.host}${request.path}-${json.method}-${json.id}`);
      _resolve(resp);
    }
    function reject(err) {
      console.timeEnd(`rpc time ${request.host}${request.path}-${json.method}-${json.id}`);
    ...
    request.once("response", function (resp) {
      ...
      resp.on("end", function () {
        ...
        if (json.method === "eth_getLogs") {
          const jsonStr = strings_1.toUtf8String(response.body);

          console.log(
            `${request.host}${request.path}-${json.method}-${json.id} req: ${JSON.stringify(json)}, resp: ${jsonStr}`,
          );
        }
        resolve(response);
      });
    ...
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          ...
          const jsonStr = strings_1.toUtf8String(options.body);
          const json = JSON.parse(jsonStr);
          console.time(`rpc time ${req.host}${req.path}-${json.method}-${json.id}`);
          if (options.body) {
            req.write(Buffer.from(options.body));
          }
          req.end();
          return [4 /*yield*/, getResponse(req, json)];
        case 1:
          response = _a.sent();
